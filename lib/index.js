...
Spreadsheet.prototype.request = function(opts, callback) {
  ...
  this.log(("requesting " + JSON.stringify(opts, null, 4)).grey);
  request(opts, function(err, response, body) {
    _this.log(("response keys: " + _.keys(response)).grey);
    _this.log(("response.statusCode: " + response.statusCode).grey);
//    _this.log(("request body: " + JSON.stringify(body, null, 4)).grey);
    //show error
    if(err)
      return callback(err);
...
//    result = parser.toJson(result, {object: true});
    result = JSON.parse(parser.toJson(result), function(key, value) {
        if (key.indexOf(':') !== -1) {
    ...
    var info = {
      ...
      worksheetTitle: result.feed.title || null,
